...
void init_db_serv_conn(serv_info_t* server_list, uint32_t server_count, uint32_t concur_conn_cnt)
{
	...
	SPDLOG_ERROR("DB server connection index for login business: [0, %u), for other business: [%u, %u) ",
...
void CDBServConn::_HandleValidateResponse(CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleValidateResp, user_name=%s, result=%d", login_name.c_str(), result);
    
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserByloginName(login_name);
    CMsgConn* pMsgConn = NULL;
    if (!pImUser) {
        SPDLOG_ERROR("ImUser for user_name=%s not exist", login_name.c_str());
        return;
    } else {
        pMsgConn = pImUser->GetUnValidateMsgConn(attach_data.GetHandle());
        if (!pMsgConn || pMsgConn->IsOpen()) {
            SPDLOG_ERROR("no such conn is validated, user_name=%s", login_name.c_str());
            return;
        }
    }
    ...
    if (result == 0)
    {
        ...
        SPDLOG_ERROR("user_name: %s, uid: %d", login_name.c_str(), user_id);
...
void CDBServConn::_HandleRecentSessionResponse(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandleRecentSessionResponse, userId=%u, session_cnt=%u", user_id, session_cnt);
...
void CDBServConn::_HandleAllUserResponse(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandleAllUserResponse, userId=%u, latest_update_time=%u, user_cnt=%u", user_id, latest_update_time, user_cnt);
...
void CDBServConn::_HandleGetMsgListResponse(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandleGetMsgListResponse, userId=%u, session_type=%u, opposite_user_id=%u, msg_id_begin=%u, cnt=%u.", user_id, session_type, session_id, msg_id_begin, msg_cnt);
...
void CDBServConn::_HandleGetMsgByIdResponse(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandleGetMsgByIdResponse, userId=%u, session_type=%u, opposite_user_id=%u, cnt=%u.", user_id, session_type, session_id, msg_cnt);
...
void CDBServConn::_HandleMsgData(CImPdu *pPdu)
{
    ...
    if (msg_id == 0) {
        SPDLOG_ERROR("HandleMsgData, write db failed, %u->%u.", from_user_id, to_user_id);
        return;
    }
    ...
    SPDLOG_ERROR("HandleMsgData, from_user_id=%u, to_user_id=%u, msg_id=%u.", from_user_id, to_user_id, msg_id);
...
void CDBServConn::_HandleGetLatestMsgIDRsp(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandleUnreadMsgCntResp, userId=%u, session_id=%u, session_type=%u, latest_msg_id=%u.",
...
void CDBServConn::_HandleUnreadMsgCountResponse(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("HandleUnreadMsgCntResp, userId=%u, total_cnt=%u, user_unread_cnt=%u.", user_id,
...
void CDBServConn::_HandleUsersInfoResponse(CImPdu* pPdu)
{
    ...
    SPDLOG_ERROR("HandleUsersInfoResp, user_id=%u, user_cnt=%u.", user_id, user_cnt);
...
void CDBServConn::_HandleRemoveSessionResponse(CImPdu* pPdu)
{
	...
	uint32_t session_type = msg.session_type();
	uint32_t session_id = msg.session_id();
	SPDLOG_ERROR("HandleRemoveSessionResp, req_id=%u, result=%u, session_id=%u, type=%u.",
...
void CDBServConn::_HandleChangeAvatarResponse(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("HandleChangeAvatarResp, user_id=%u, result=%u.", user_id, result);
...
void CDBServConn::_HandleDepartmentResponse(CImPdu *pPdu)
{
    ...
    SPDLOG_ERROR("HandleDepartmentResponse, user_id=%u, latest_update_time=%u, dept_cnt=%u.", user_id, latest_update_time, dept_cnt);
...
void CDBServConn::_HandleSetDeviceTokenResponse(CImPdu *pPdu)
{
    ...
    uint32_t user_id = msg.user_id();
    SPDLOG_ERROR("HandleSetDeviceTokenResponse, user_id = %u.", user_id);
}
...
void CDBServConn::_HandleGetDeviceTokenResponse(CImPdu *pPdu)
{
    ...
    if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_TEXT || msg_type == IM::BaseDefine::MSG_TYPE_GROUP_TEXT)
    {
        ...
        {
            SPDLOG_ERROR("HandleGetDeviceTokenResponse, decrypt msg failed, from_id: %u, to_id: %u, msg_type: %u.", from_id, to_id, msg_type);
            return;
        }
        pAes->Free(msg_out);
    }
    ...
    SPDLOG_ERROR("HandleGetDeviceTokenResponse, user_token_cnt = %u.", user_token_cnt);
    ...
    for (uint32_t i = 0; i < user_token_cnt; i++)
    {
        ...
        SPDLOG_ERROR("HandleGetDeviceTokenResponse, user_id = %u, device_token = %s, push_cnt = %u, client_type = %u.",
        ...
        if (pUser)
        {
            ...
            if (pUser->GetPCloginStatus() == IM_PC_logIN_STATUS_ON)
            {
                ...
                SPDLOG_ERROR("HandleGetDeviceTokenResponse, user id: %d, push type: silent.", user_id);
            }
            else
            {
                user_token_tmp->set_push_type(IM_PUSH_TYPE_NORMAL);
                SPDLOG_ERROR("HandleGetDeviceTokenResponse, user id: %d, push type: normal.", user_id);
            }
        }
        else
        {
...
void CDBServConn::_HandleChangeSignInfoResponse(CImPdu* pPdu) {
        ...
        SPDLOG_ERROR("HandleChangeSignInfoResp: user_id=%u, result=%u.", user_id, result);
        ...
        if (pMsgConn && pMsgConn->IsOpen()) {
                ...
                pMsgConn->SendPdu(pPdu);
                   ...
                   SPDLOG_ERROR("HandleChangeSignInfoResp: can't found msg_conn by user_id = %u, handle = %u", user_id, handle);
...
void CDBServConn::_HandlePushShieldResponse(CImPdu* pPdu) {
    ...
    SPDLOG_ERROR("_HandlePushShieldResponse: user_id=%u, result=%u.", user_id, result);
    ...
    if (pMsgConn && pMsgConn->IsOpen()) {
        ...
        SPDLOG_ERROR("_HandlePushShieldResponse: can't found msg_conn by user_id = %u, handle = %u", user_id, handle);
    }
}
...
void CDBServConn::_HandleQueryPushShieldResponse(CImPdu* pPdu) {
    ...
    SPDLOG_ERROR("_HandleQueryPushShieldResponse: user_id=%u, result=%u.", user_id, result);
    ...
    if (pMsgConn && pMsgConn->IsOpen()) {
        ...
        SPDLOG_ERROR("_HandleQueryPushShieldResponse: can't found msg_conn by user_id = %u, handle = %u", user_id, handle);
    }
}
