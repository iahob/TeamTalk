...
void CFileModel::getOfflineFile(uint32_t userId, list<IM::BaseDefine::OfflineFileInfo>& lsOffline)
{
    ...
    if (pDBConn)
    {
        ...
        {
            SPDLOG_ERROR("no result for:%s", strSql.c_str());
        }
...
void CFileModel::delOfflineFile(uint32_t fromId, uint32_t toId, string& taskId)
{
    ...
    if (pDBConn)
    {
        ...
        {
            SPDLOG_ERROR("delete offline file success.%d->%d:%s", fromId, toId, taskId.c_str());
        }
        else
        {
            SPDLOG_ERROR("delete offline file failed.%d->%d:%s", fromId, toId, taskId.c_str());
        }
        pDBManager->RelDBConn(pDBConn);
    }
    else
