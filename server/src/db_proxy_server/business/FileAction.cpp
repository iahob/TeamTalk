...
namespace DB_PROXY {
    ...
    //            *pInfo = *it;
            ...
            SPDLOG_ERROR("userId=%u, count=%u", nUserId, msgResp.offline_file_list_size());
    ...
    void addOfflineFile(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            SPDLOG_ERROR("fromId=%u, toId=%u, taskId=%s, fileName=%s, fileSize=%u", nUserId, nToId, strTaskId.c_str(), strFileName.c_str(), nFileSize);
        }
    }
    ...
    void delOfflineFile(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
            ...
            SPDLOG_ERROR("fromId=%u, toId=%u, taskId=%s", nUserId, nToId, strTaskId.c_str());
        }
    }
};