        ...
        if (pDBConn)
        {
            ...
            if (pResultSet)
            {
                while (pResultSet->Next())
                {
                    ...
                    {
                        SPDLOG_ERROR("invalid msgType. userId=%u, peerId=%u, msgId=%u, msgCnt=%u, msgType=%u", nUserId, nPeerId, nMsgId, nMsgCnt, nMsgType);
                    }
                }
                delete pResultSet;
            }
            ...
            {
                SPDLOG_ERROR("no result set: %s", strSql.c_str());
            }
...
bool CMessageModel::sendMessage(uint32_t nRelateId, uint32_t nFromId, uint32_t nToId, IM::BaseDefine::MsgType nMsgType, uint32_t nCreateTime, uint32_t nMsgId, string& strMsgContent)
{
    ...
    if (nFromId == 0 || nToId == 0) {
        SPDLOG_ERROR("invalied userId.%u->%u", nFromId, nToId);
        return bRet;
    }
...
void CMessageModel::incMsgCount(uint32_t nFromId, uint32_t nToId)
{
	...
	if (pCacheConn) {
		...
		SPDLOG_ERROR("no cache connection to increase unread count: %d->%d", nFromId, nToId);
	}
}
    ...
    if (pCacheConn)
    {
        ...
        if(bRet)
        {
            ...
            for (auto it = mapUnread.begin(); it != mapUnread.end(); it++) {
                ...
                {
                    SPDLOG_ERROR("invalid msgType. userId=%u, peerId=%u, msgType=%u", nUserId, cUnreadInfo.session_id(), nMsgType);
                }
            }
        }
        else
...
void CMessageModel::getMsgByMsgId(uint32_t nUserId, uint32_t nPeerId, const list<uint32_t> &lsMsgId, list<IM::BaseDefine::MsgInfo> &lsMsg)
{
    ...
    if(nRelateId == INVALID_VALUE)
    {
        SPDLOG_ERROR("invalid relation id between %u and %u", nUserId, nPeerId);
        return;
    }
    ...
    if (pDBConn)
    {
        ...
        if (pResultSet)
        {
            while (pResultSet->Next())
            {
                ...
                {
                    SPDLOG_ERROR("invalid msgType. userId=%u, peerId=%u, msgType=%u, msgId=%u", nUserId, nPeerId, nMsgType, msg.msg_id());
                }
            }
            delete pResultSet;
        }
