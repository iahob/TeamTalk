...
int CacheConn::Init()
{
	...
	if (!m_pContext || m_pContext->err) {
		if (m_pContext) {
            SPDLOG_ERROR("redisConnect failed: %s", m_pContext->errstr);
			redisFree(m_pContext);
			m_pContext = NULL;
		} else {
...
string CacheConn::get(string key)
{
	...
	if (!reply) {
        SPDLOG_ERROR("redisCommand failed:%s", m_pContext->errstr);
		redisFree(m_pContext);
		m_pContext = NULL;
		return value;
	}
...
long CacheConn::incrBy(string key, long value)
{
    ...
    if(!reply)
    {
        SPDLOG_ERROR("redis Command failed:%s", m_pContext->errstr);
        redisFree(m_pContext);
        m_pContext = NULL;
        return -1;
    }
