	...
	if (m_handle != NETLIB_INVALID_HANDLE) {
		...
		if (m_conn_type == logIN_CONN_TYPE_CLIENT) {
			...
			if (it != g_msg_serv_info.end()) {
				...
				SPDLOG_ERROR("onclose from MsgServer: %s:%u ", pMsgServInfo->hostname.c_str(), pMsgServInfo->port);
				delete pMsgServInfo;
...
void CloginConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        default:
            SPDLOG_ERROR("wrong msg, cmd id=%d ", pPdu->GetCommandId());
            break;
	}
}
...
void CloginConn::_HandleMsgServInfo(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("MsgServInfo, ip_addr1=%s, ip_addr2=%s, port=%d, max_conn_cnt=%d, cur_conn_cnt=%d, "\
		"hostname: %s. ",
...
void CloginConn::_HandleUserCntUpdate(CImPdu* pPdu)
{
	...
	if (it != g_msg_serv_info.end()) {
		...
		SPDLOG_ERROR("%s:%d, cur_cnt=%u, total_cnt=%u ", pMsgServInfo->hostname.c_str(),
