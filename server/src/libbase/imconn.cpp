...
void imconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    ...
    //SPDLOG_ERROR("msg=%d, handle=%d ", msg, handle);
    ...
    switch (msg)
    {
        ...
        case NETLIB_MSG_CLOSE:
            ...
            break;
        default:
            SPDLOG_ERROR("!!!imconn_callback error msg: %d ", msg);
            break;
    }
...
CImConn::~CImConn()
{
    //SPDLOG_ERROR("CImConn::~CImConn, handle=%d ", m_handle);
}
...
int CImConn::Send(void* data, int len)
{
    ...
    if (remain > 0)
    {
        ...
        m_busy = true;
        SPDLOG_ERROR("send busy, remain=%d ", m_out_buf.GetWriteOffset());
    }
    else
    {
        OnWriteCompelete();
...
void CImConn::OnWrite()
{
    ...
    SPDLOG_INFO("onWrite, remain=%d ", m_out_buf.GetWriteOffset());
}
