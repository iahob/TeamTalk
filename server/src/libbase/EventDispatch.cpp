...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			SPDLOG_ERROR("select failed, error code: %d", GetLastError());
			Sleep(MIN_TIMER_DURATION);
		...
		{
			//SPDLOG_ERROR("select return read count=%d\n", read_set.fd_count);
		...
		{
			//SPDLOG_ERROR("select return write count=%d\n", write_set.fd_count);
		...
		{
			//SPDLOG_ERROR("select return exception count=%d\n", excep_set.fd_count);
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		SPDLOG_ERROR("epoll_ctl() failed, errno=%d", errno);
	}
}
