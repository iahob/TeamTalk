...
CBaseSocket::~CBaseSocket()
{
	//SPDLOG_ERROR("CBaseSocket::~CBaseSocket, socket=%d\n", m_socket);
}
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	if (ret == SOCKET_ERROR)
	{
		SPDLOG_ERROR("bind failed, err_code=%d", _GetErrorCode());
	...
	if (ret == SOCKET_ERROR)
	{
	//	SPDLOG_ERROR("listen failed, err_code=%d", _GetErrorCode());
        SPDLOG_INFO("CBaseSocket::Listen on %s:%d", server_ip, port);
	...
	//SPDLOG_ERROR("CBaseSocket::Listen on %s:%d", server_ip, port);
	AddBaseSocket(this);
...
{
	SPDLOG_INFO("CBaseSocket::Connect, server_ip=%s, port=%d", server_ip, port);
	...
	if (m_socket == INVALID_SOCKET)
	{
		SPDLOG_ERROR("socket failed, err_code=%d", _GetErrorCode());
	...
	{	
		SPDLOG_ERROR("connect failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_INVALID_HANDLE;
	}
...
#endif
			ret = 0;
			//SPDLOG_ERROR("socket send block fd=%d", m_socket);
		}
		else
		{
			SPDLOG_ERROR("!!!send failed, error code: %d", err_code);
		}
	}
...
void CBaseSocket::SetSendBufSize(uint32_t send_size)
{
	...
	if (ret == SOCKET_ERROR) {
		SPDLOG_ERROR("set SO_SNDBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, &size, &len);
	SPDLOG_INFO("socket=%d send_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size)
{
	...
	if (ret == SOCKET_ERROR) {
		SPDLOG_ERROR("set SO_RCVBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, &size, &len);
	SPDLOG_INFO("socket=%d recv_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		SPDLOG_ERROR("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		SPDLOG_ERROR("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		SPDLOG_ERROR("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr)
{
	...
	if (pAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		if (host == NULL)
		{
			SPDLOG_ERROR("gethostbyname failed, ip=%s", ip);
			return;
		}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		SPDLOG_INFO("AcceptNewSocket, socket=%d from %s:%d\n", fd, ip_str, port);
