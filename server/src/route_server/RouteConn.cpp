...
{
	SPDLOG_ERROR("MsgServer onclose: handle=%d ", m_handle);
	Close();
}
...
void CRouteConn::HandlePdu(CImPdu* pPdu)
{
	...
	default:
		SPDLOG_ERROR("CRouteConn::HandlePdu, wrong cmd id: %d ", pPdu->GetCommandId());
		break;
	}
}
...
void CRouteConn::_HandleOnlineUserInfo(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("HandleOnlineUserInfo, user_cnt=%u ", user_count);
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("HandleUserStatusUpdate, status=%u, uid=%u, client_type=%u ", user_status, user_id, client_type);
...
void CRouteConn::_HandleRoleSet(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("HandleRoleSet, master=%u, handle=%u ", master, m_handle);
	if (master == 1) {
		m_bMaster = true;
	} else {
		m_bMaster = false;
...
void CRouteConn::_HandleUsersStatusRequest(CImPdu* pPdu)
{
	...
	SPDLOG_ERROR("HandleUserStatusReq, req_id=%u, query_count=%u ", request_id, query_count);
